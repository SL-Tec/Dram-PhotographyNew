...
// Function to toggle mobile menu with animation
function toggleMobileMenu() {
    isMobileMenuOpen = !isMobileMenuOpen;
    
    // Update button icon with rotation
    const icon = mobileMenuButton.querySelector('i');
    icon.style.transition = 'transform 0.3s ease';
    icon.style.transform = isMobileMenuOpen ? 'rotate(90deg)' : 'rotate(0deg)';
    
    // Toggle menu with slide animation
    if (isMobileMenuOpen) {
...
// Update active navigation link on scroll with smooth transition
...
function updateActiveLink() {
    ...
    navLinks.forEach(link => {
        link.classList.remove('text-blue-600', 'dark:text-blue-400', 'bg-gray-100', 'dark:bg-gray-800');
        if (link.getAttribute('href').substring(1) === current) {
            link.classList.add('text-blue-600', 'dark:text-blue-400', 'bg-gray-100', 'dark:bg-gray-800');
        }
    });
}
